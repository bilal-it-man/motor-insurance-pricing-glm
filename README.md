...
- The pure-premium Gini doesn't hold up under cross-validation (see
  Results above). That's not something a bigger sample of exactly this size
  would necessarily fix - it's a heavy-tail problem, and the practical way
  around it is either a much larger claims dataset or, as this project ended
  up doing, judging frequency and severity on their own terms rather than
  only as one blended number.
